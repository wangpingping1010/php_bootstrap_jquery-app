...
class Inventory_summary extends Report
{
	...
	public function getSummaryData()
	{
		...
		if ($this->params['supplier'])
		{
			$suplier = 'Where supplier_id in ('.$this->params['supplier'].')';
		}
        $category_id = '';
		if ($this->params['category_id'])
		{
		    ...
		    if ($this->params['supplier'])
    		{
    		    ...
    		    $category_id = 'Where phppos_items.category_id  in ('.$category_ids_string.')';
    		}
		...
		if (isset($this->params['item_name']) && $this->params['item_name'])
		{
			if (!$this->params['category_id'] && !$this->params['supplier'])
			{
				$items_where = ' WHERE phppos_items.name LIKE "%'.$this->db->escape_like_str($this->params['item_name']).'%" ';
			}
			else
			{
		...
		else
		{
		    ...
		    $query = 'select 
                ...
                ifnull(q.quantity,0) + ifnull(v.varaition_quantity,0) as total_items_in_inventory ,
                
                (ifnull(q.quantity,0) + ifnull(v.varaition_quantity,0)) * cost_price as inventory_total,
                (ifnull(q.quantity,0) + ifnull(v.varaition_quantity,0)) * unit_price  as inventory_sale_total
                ,
                q_unit,
                v_unit,
                q_cost_price
                ...
                select item_id  ,sum(quantity) quantity 
                    ,sum(cost_price) q_cost_price
                    ,sum(unit_price) q_unit
                 from phppos_location_items 
                where item_id not in (select item_id from phppos_item_variations )
                and  phppos_location_items.`location_id` IN ('.$location_ids_string.') '.$in_stock.' '.$out_stock.'
                group by item_id ) q on phppos_items.item_id=q.item_id  and phppos_items.deleted =0 and is_service != 1
                
                left JOIN(
                    
                SELECT item_id , sum(quantity) varaition_quantity
                    
                ,sum(phppos_item_variations.cost_price) v_cost_price
                    ,sum(phppos_item_variations.unit_price) v_unit 
                ...
                WHERE `phppos_location_item_variations`.`location_id` IN('.$location_ids_string.')  '.$in_stock.' '.$out_stock.'
                GROUP BY item_id 
                    ) v 
                 on phppos_items.item_id=v.item_id and phppos_items.deleted =0 and is_service != 1
